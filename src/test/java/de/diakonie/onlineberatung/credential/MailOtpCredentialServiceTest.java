...
public class MailOtpCredentialServiceTest {
  ...
  public void activate_should_activate_and_reset_failed_verifications() {
    ...
    assertThat(credential.getOtp().getFailedVerifications()).isEqualTo(0);
    assertThat(credential.getOtp().getCode()).isEqualTo(INVALIDATED);
  ...
  public void incrementFailedAttempts_should_increment_failed_attempts() {
    ...
    assertThat(credential.getOtp().getFailedVerifications()).isEqualTo(1);
    verify(credentialProvider).updateCredential(realm, user, credential);
  }
}